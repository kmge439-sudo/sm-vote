...
export default function App() {
  ...
  const syncAllData = async () => {
    ...
    if (!confirm(`현재 코드에서 감지된 학생은 총 ${count}명입니다. DB로 전송하시겠습니까?`)) return;
    ...
    try {
      ...
      alert(`${count}명 명단 DB 이관 완료!`);
    } catch (e) { alert("오류: " + e.message); }
    finally { setIsSyncing(false); }
  };

  const deleteStudent = async (studentKey) => {
    if (!confirm(`${studentKey} 학생의 모든 투표 데이터와 인증 정보를 삭제하시겠습니까?`)) return;
    try {
      ...
      <div className="max-w-4xl w-full">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-widest shadow-lg">
            <Lock size={12} /> SECURED SYSTEM V6.3
          </div>
            ...
            {step === 6 && (
              <div className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-100 animate-in zoom-in duration-500 text-slate-900">
                ...
                <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100 mb-8 flex items-start gap-3 text-slate-900">
                  ...
                  <p className="text-sm text-blue-900 font-bold leading-relaxed">본 투표는 <span className="underline decoration-2">비밀 투표</span>로 진행됩니다. 제출 후에는 관리자를 포함한 그 누구도 투표 내용을 확인할 수 없으니 안심하고 제출해 주세요.</p>
                </div>
                            ...
                            return (
                              <tr key={s.key} className="hover:bg-slate-50 text-slate-900">
                                ...
                                <td className="p-4 text-center flex items-center justify-center gap-3">
                                  {voterData && <button onClick={()=>setResetConfirm(s.key)} title="개별 리셋" className="text-amber-500 hover:scale-110 transition-transform"><RotateCcw size={14}/></button>}
                                  <button onClick={()=>deleteStudent(s.key)} title="명단 삭제" className="text-rose-500 hover:scale-110 transition-transform"><Trash2 size={14}/></button>
                                </td>
                              </tr>
                            );
                          })}
                    ...
                    <div className="bg-blue-50 p-8 rounded-[2.5rem] border border-blue-100">
                      <div className="flex justify-between items-start mb-6">
                        <div>
                          ...
                          <p className="text-xs text-blue-600 font-bold leading-tight">코드에 입력된 학생 데이터를 서버(Firebase)로 강제 전송합니다.</p>
                        </div>
                        <div className="bg-blue-600 text-white px-4 py-2 rounded-2xl font-black text-xs">
                          코드 내 감지 학생: {Object.keys(parseStudentVault()).length}명
                        </div>
                      </div>
                      <button onClick={syncAllData} disabled={isSyncing} className="w-full py-4 bg-blue-600 text-white rounded-3xl font-black shadow-lg flex items-center justify-center gap-2 font-sans active:scale-95 transition-transform">
                        {isSyncing ? <RefreshCw className="animate-spin" /> : <Save />} 전교생 849명 명단 DB 이관 실행
                      </button>
                    </div>

                    <button onClick={() => setShowResetAllModal(true)} className="w-full py-4 bg-rose-50 text-rose-600 border border-rose-200 rounded-3xl font-black text-sm flex items-center justify-center gap-2 text-rose-600 font-sans hover:bg-rose-100"><Trash2 size={16}/> 전체 투표 데이터 초기화 (신중히 사용)</button>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="mt-20 pt-10 border-t border-slate-200 text-center text-slate-900 font-sans">
          {!showAdminPanel && (
            <button onClick={() => setShowAdminPanel(true)} className="flex items-center gap-2 mx-auto text-slate-400 hover:text-blue-600 font-bold text-sm transition-colors uppercase tracking-[0.2em] font-sans"><BarChart3 size={16} /> Admin Suite V6.3</button>
          )}
        </div>
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900">EMS Terminal V6.3 Master Final</footer>
      </div>
    </div>
  );
}
