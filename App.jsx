...
export default function App() {
  ...
  const totalStudentsList = useMemo(() => {
    return Object.keys(AUTH_CODES).map(key => ({
  ...
  const verifyStudent = async () => {
    ...
    // local AUTH_CODES에서 확인
    const expectedCode = AUTH_CODES[studentKey];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4 font-sans text-slate-900 selection:bg-blue-100 select-none">
      <div className="max-w-3xl w-full">
                      ...
                      <div className="bg-white border rounded-2xl overflow-hidden max-h-[400px] overflow-y-auto shadow-inner text-slate-900 font-sans text-slate-900">
                        <table className="w-full text-left text-[10px] font-sans text-slate-900">
                          <thead className="bg-slate-50 font-black sticky top-0 text-slate-900">
                            <tr><th className="p-3 text-slate-900">학생 정보</th><th className="p-3 text-center text-slate-900">상태</th><th className="p-3 text-center text-slate-900">리셋</th></tr>
                          </thead>
        ...
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900 text-slate-900">EMS Terminal V5.2 Final Build</footer>
      </div>
    </div>
  );
}
