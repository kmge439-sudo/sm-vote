...
export default function App() {
  ...
  const syncAuthVault = async () => {
    if (!confirm("코드의 명단을 금고(DB)로 동기화하시겠습니까?")) return;
    setIsSyncing(true);
    try {
      const batchSize = 300;
  ...
  const handleExportVoters = () => {
    ...
    Object.entries(FINAL_AUTH_CODES_MAP).forEach(([id, data]) => {
      ...
      rows.push([id[0], id[1], id.substring(2), data.name, v ? "완료" : "미참여", timeStr]);
    });
      ...
      <div className="max-w-3xl w-full text-slate-900">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-[0.2em] shadow-lg text-slate-100">
            <Lock size={12} /> SECURED SYSTEM V5.8
          </div>
        ...
        {!showAdminPanel ? (
              ...
              <div className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-100 animate-in zoom-in duration-500 text-slate-900">
                ...
                <div className="space-y-4 mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900">
                    ...
                    <div className="p-5 bg-blue-50 border border-blue-100 rounded-2xl font-black text-slate-900"><p className="text-[10px] text-blue-600 mb-1 uppercase tracking-widest font-black">1학년 부회장</p>{selectedVP1?.name}</div>
                    <div className="p-5 bg-indigo-50 border border-indigo-100 rounded-2xl font-black text-slate-900"><p className="text-[10px] text-indigo-600 mb-1 uppercase tracking-widest font-black">2학년 부회장</p>{selectedVP2?.name}</div>
                    <div className="p-5 bg-purple-50 border border-purple-100 rounded-2xl font-black text-slate-900"><p className="text-[10px] text-purple-600 mb-1 uppercase tracking-widest font-black">3학년 부회장</p>{selectedVP3?.name}</div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden relative animate-in slide-in-from-bottom-4 text-left text-slate-900">
              {(resetConfirm || showResetAllModal) && (
                <div className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 text-center text-white">
                  <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in text-slate-900">
                    ...
                    <p className="text-sm text-slate-500 font-bold mb-6 text-slate-900">{resetConfirm ? '해당 학생의 투표 데이터를 삭제하시겠습니까?' : '주의! 모든 투표 결과가 삭제됩니다.'}</p>
                    <div className="flex gap-3 text-slate-900"><button onClick={() => {setResetConfirm(null); setShowResetAllModal(false);}} className="flex-1 py-3 bg-slate-100 rounded-xl font-bold text-slate-900">취소</button><button onClick={resetConfirm ? () => handleResetVoter(resetConfirm) : handleResetAllVotes} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-bold shadow-lg text-slate-100">실행</button></div>
                  </div>
                </div>
              )}
              ...
              {!isAdminAuthenticated ? (
                ...
                <div className="p-8 space-y-8 animate-in fade-in duration-500 text-slate-900 font-sans">
                  ...
                  {adminTab === 'stats' ? (
                    <div className="space-y-10">
                      <div className="flex flex-wrap justify-between items-center gap-4">
                        <p className="font-black text-slate-700 text-sm">전체 투표율: <span className="text-blue-600 text-xl font-sans">{((stats.total / Object.keys(FINAL_AUTH_CODES_MAP).length) * 100).toFixed(1)}%</span> ({stats.total}/{Object.keys(FINAL_AUTH_CODES_MAP).length}명)</p>
                      ...
                      <div className="bg-white border rounded-2xl overflow-hidden max-h-[300px] overflow-y-auto shadow-inner text-slate-900 font-sans">
                        <table className="w-full text-left text-[10px] font-sans text-slate-900">
                          <thead className="bg-slate-50 font-black sticky top-0 text-slate-900">
                            <tr><th className="p-3 text-slate-900">학생 정보</th><th className="p-3 text-center text-slate-900">상태</th><th className="p-3 text-center text-slate-900">투표 시간</th><th className="p-3 text-center text-slate-900">리셋</th></tr>
                          </thead>
                          <tbody className="divide-y font-sans">
                            {filteredParticipationList.map(([id, data]) => {
                              const voterData = dbVoters[id + data.name];
                              const hasVoted = !!voterData;
                              return (
                                <tr key={id} className="hover:bg-slate-50 transition-colors">
                                  <td className="p-3 font-black text-slate-900">{id[0]}-{id[1]}-{id.substring(2)} {data.name}</td>
                                  ...
                                  <td className="p-3 text-center">
                                    {hasVoted && (
                                      <button onClick={() => setResetConfirm(id + data.name)} className="text-amber-500 hover:scale-110 text-amber-500">
                                        <RotateCcw size={14} />
                                      </button>
                                    )}
                                  </td>
        ...
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900 text-slate-900">EMS Terminal V5.8 Final Build</footer>
      </div>
    </div>
  );
}
