...
export default function App() {
  ...
  const syncAllData = async () => {
    if (!confirm("시스템을 초기 상태로 동기화하시겠습니까?")) return;
    setIsSyncing(true);
    try {
      ...
      <div className="max-w-4xl w-full">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-widest shadow-lg">
            <Lock size={12} /> SECURED SYSTEM V6.0
          </div>
                ...
                {adminTab === 'students' && (
                  <div className="space-y-6 text-slate-900">
                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-slate-900">
                      <h4 className="font-black text-sm mb-4 flex items-center gap-2"><UserPlus size={18}/> 신규 학생 등록</h4>
                        ...
                        <div className="space-y-4">
                          {dbCandidates[cat].map((c, i) => (
                            <div key={c.id} className="bg-white p-4 rounded-xl shadow-sm space-y-2 border text-slate-900">
                              ...
                              <input className="w-full text-xs font-bold text-slate-500 bg-transparent outline-none text-slate-900" value={c.slogan} onChange={async (e)=>{
                          ...
                          <button onClick={async () => {
                            ...
                            await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated);
                          }} className="w-full py-2 border-2 border-dashed rounded-xl text-slate-400 font-black text-[10px] hover:bg-slate-100">+ 추가</button>
                        </div>
                      </div>
                    ))}
                  </div>
