...
import { 
  User, CheckCircle, Award, Users, AlertCircle, Lock, BarChart3, RefreshCw, KeyRound, ShieldCheck, X, Search, RotateCcw, Trash2, AlertTriangle, Fingerprint, ChevronRight, ChevronLeft, ShieldAlert
} from 'lucide-react';
...
export default function App() {
  ...
  const totalStudentsList = useMemo(() => {
    return Object.keys(SYNC_DATA).map(key => ({
  ...
  const verifyStudent = async () => {
    ...
    try {
      // 1. 서버(DB)에서 해당 학생의 코드만 가져오기 (전체 목록은 몰라도 됨)
      const vaultDoc = await getDoc(doc(db, 'artifacts', appId, 'public', 'data', 'authVault', studentKey));
      
      // 만약 DB 동기화 전이라면 코드의 SYNC_DATA에서 찾음 (첫 사용 시 대비)
      const expectedCode = vaultDoc.exists() ? vaultDoc.data().code : SYNC_DATA[studentKey];
  ...
  const syncAuthCodesToDB = async () => {
    if (!confirm("849명의 인증 코드를 서버 금고로 동기화하시겠습니까?")) return;
    setIsSyncing(true);
    try {
      const batch = writeBatch(db);
      Object.entries(SYNC_DATA).forEach(([key, val]) => {
        const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'authVault', key);
        batch.set(docRef, { code: val });
      });
      await batch.commit();
      alert("성공! 이제 모든 인증 코드가 서버 금고에 저장되었습니다.");
      ...
      <div className="max-w-3xl w-full">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-[0.2em] shadow-lg">
            <Lock size={12} /> SECURED SYSTEM V5.3
          </div>
              ...
              {(resetConfirm || showResetAllModal) && (
                <div className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 text-center">
                  <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in text-slate-900">
                    ...
                    <p className="text-sm text-slate-500 font-bold mb-6">{resetConfirm ? '해당 학생의 투표 데이터를 삭제하시겠습니까?' : '주의! 모든 투표 기록과 결과가 완전히 삭제됩니다.'}</p>
                    <div className="flex gap-3">
                      ...
                      <button onClick={resetConfirm ? () => handleResetVoter(resetConfirm) : handleResetAllVotes} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-bold">진행</button>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  {adminTab === 'stats' ? (
                    <div className="space-y-10">
                      <div className="flex flex-wrap justify-between items-center gap-4">
                        ...
                        <button onClick={() => setShowResetAllModal(true)} className="px-4 py-2 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl text-xs font-black flex items-center gap-2 hover:bg-rose-100"><Trash2 size={14}/> 전체 초기화</button>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                        {/* 회장 및 부회장들 집계 그래프 4개 (생략 없이 모두 표시) */}
                    ...
                    <div className="space-y-6">
                      <div className="bg-amber-50 p-6 rounded-2xl border border-amber-100">
                        <h4 className="font-black text-sm mb-2 flex items-center gap-2 text-amber-800"><ShieldAlert size={18}/> 보안 금고(DB) 관리</h4>
                        <p className="text-xs text-amber-700 font-bold mb-4 leading-tight">코드 안의 인증번호를 서버로 옮깁니다. 선거 시작 전 딱 한 번만 실행하세요.</p>
                        <button onClick={syncAuthCodesToDB} disabled={isSyncing} className="w-full py-3 bg-amber-600 text-white rounded-xl font-black text-xs shadow-lg flex items-center justify-center gap-2">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <div className="relative col-span-3"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="검색" value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} className="w-full p-3 pl-9 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-bold" /></div>
                      </div>
                            ...
                            {filteredParticipationList.map((s, idx) => {
                              ...
                              return (
                                <tr key={`${s.key}-${idx}`} className="hover:bg-slate-50 transition-colors">
                                  ...
                                  <td className="p-3 text-center">{hasVoted && (<button onClick={() => setResetConfirm(s.key)} className="text-slate-400 hover:text-rose-600"><RotateCcw size={14} /></button>)}</td>
                                </tr>
                              );
                            })}
                          </tbody>
        ...
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] text-slate-900">EMS Terminal V5.3 Final Security Build</footer>
      </div>
    </div>
  );
}
