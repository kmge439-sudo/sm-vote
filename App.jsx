...
// ==========================================
// [보안 데이터] 849명 전교생 명단 분할 (A:학번, B:이름, C:코드 규칙 반영)
// ==========================================
...
export default function App() {
  ...
  const handleExportResults = () => {
    ...
    ['president', 'vp1', 'vp2', 'vp3'].forEach(cat => {
      const candidatesList = candidates[cat];
            ...
            {step === 6 && (
              <div className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-100 animate-in zoom-in duration-500 text-slate-900">
                ...
                <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100 mb-8 flex items-start gap-3">
                  ...
                  <p className="text-sm text-blue-900 font-bold leading-relaxed">본 투표는 <span className="underline decoration-2 underline-offset-2">비밀 투표</span>로 진행됩니다. 제출 후에는 그 누구도 투표 내용을 확인할 수 없으니 안심하고 제출해 주세요.</p>
                </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden relative animate-in slide-in-from-bottom-4 text-left text-slate-900 text-slate-900">
              ...
              {!isAdminAuthenticated ? (
                <form onSubmit={handleAdminLogin} className="p-10 space-y-4">
                  ...
                  <button type="submit" className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black">인증</button>
                </form>
              ) : (
                  ...
                  {adminTab === 'stats' ? (
                    <div className="space-y-10">
                      <div className="flex flex-wrap justify-between items-center gap-4">
                        <p className="font-black text-slate-700 text-sm text-slate-900">전체 투표율: <span className="text-blue-600 text-xl font-sans">{((stats.total / Object.keys(FINAL_AUTH_CODES_MAP).length) * 100).toFixed(1)}%</span> ({stats.total}/{Object.keys(FINAL_AUTH_CODES_MAP).length}명)</p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 text-slate-900">
                        {['pres', 'vp1', 'vp2', 'vp3'].map((key) => {
                          ...
                          const currentMax = Math.max(...candidatesList.map(p => stats[key][p.id] || 0));
                          return (
                              ...
                              return (
                                <tr key={id} className="hover:bg-slate-50 transition-colors">
                                  ...
                                  <td className="p-3 text-center">
                                    {hasVoted && (
                                      <button onClick={() => setResetConfirm(voterKey)} className="text-amber-500 hover:scale-110 text-amber-500">
                                        <RotateCcw size={14} />
                                      </button>
                                    )}
                                  </td>
