...
import { 
  User, CheckCircle, Award, Users, AlertCircle, Lock, BarChart3, RefreshCw, KeyRound, ShieldCheck, X, Search, RotateCcw, Trash2, AlertTriangle, Fingerprint, ChevronRight, ChevronLeft, ShieldAlert, PlayCircle, StopCircle, Timer, Clock, Trophy, Download, UserPlus, Save, ClipboardCheck, Plus, UserMinus, Edit3
} from 'lucide-react';
...
// ==========================================
// [보안 데이터] 850명 전교생 명단 고정 관리 (3207 김지원 반영)
// ==========================================
  ...
  if (isSubmitted) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 text-slate-900">
        <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-10 text-center border border-slate-100 text-slate-900 text-slate-900">
          ...
          <p className="text-slate-600 mb-8 font-medium text-slate-600">참여해 주셔서 감사합니다.</p>
          <button onClick={() => window.location.reload()} className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black transition-all active:scale-95 text-slate-100 font-sans text-slate-100">닫기</button>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl w-full text-slate-900">
        <div className="text-center mb-12 text-slate-900">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-[0.2em] shadow-lg text-slate-100">
            <Lock size={12} /> SECURED SYSTEM V8.5
          </div>
                ...
                {votingState === 'active' ? (
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-4 text-slate-900">
                      ...
                      <input type="number" placeholder="번" value={userData.number} onChange={(e) => setUserData({...userData, number: e.target.value})} className="p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-center outline-none text-slate-900 text-slate-900" />
                    </div>
                    <input type="text" placeholder="이름" value={userData.name} onChange={(e) => setUserData({...userData, name: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-xl px-8 mb-4 outline-none focus:border-blue-500 text-slate-900 font-sans text-slate-900" />
                    <div className="relative mb-6 text-slate-900">
                      ...
                      <input type="text" placeholder="인증 코드 입력" value={userData.authCode} onChange={(e) => setUserData({...userData, authCode: e.target.value})} className="w-full p-5 pl-16 bg-blue-50 border-2 border-blue-100 rounded-2xl font-black text-xl outline-none focus:border-blue-500 text-slate-900 font-sans text-slate-900" />
                    </div>
                  ...
                  <div className="text-center py-10 text-slate-900">
                    {votingState === 'ready' ? (
                      ...
                      <><StopCircle size={64} className="mx-auto text-rose-500 mb-6" /><h2 className="text-2xl font-black mb-4 text-slate-900 text-slate-900">투표 종료</h2><p className="text-slate-500 font-bold font-sans text-slate-900">2026학년도 선거 투표가 마감되었습니다.</p></>
                    )}
                  </div>
                )}
              </div>
            ...
            {[2, 3, 4, 5].includes(step) && dbCandidates && (
              <div className="animate-in slide-in-from-right-8 duration-500 text-slate-900 text-slate-900">
                <div className="flex justify-between items-end mb-8 text-slate-900 text-slate-900">
                  ...
                  <span className="text-xs font-black text-slate-400 tracking-widest uppercase font-sans text-slate-900 text-slate-400">Step {step-1} / 4</span>
                </div>
                  ...
                  {(dbCandidates[step === 2 ? 'president' : `vp${step-2}`] || []).map(c => {
                    ...
                    return (
                      <div key={c.id} onClick={() => {
                        ...
                        <div>
                          <span className="text-[10px] font-black bg-slate-100 px-3 py-1 rounded-lg uppercase tracking-wider text-slate-900 text-slate-900 text-slate-900">기호 {c.id % 100}번</span>
                          <h3 className="text-xl font-black mt-2 text-slate-900 text-slate-900 text-slate-900">{c.name}</h3>
                          <p className="text-slate-500 font-bold text-sm italic font-sans text-slate-500 text-slate-500">"{c.slogan}"</p>
                        </div>
                ...
                <div className="flex gap-4 mt-10 text-slate-900">
                  <button onClick={() => setStep(step - 1)} className="flex-1 py-4 bg-white border border-slate-200 rounded-2xl font-black transition-all active:scale-95 flex items-center justify-center gap-2 text-slate-900 font-sans text-slate-900"><ChevronLeft size={18}/>이전</button>
                  <button disabled={(step===2 && !selectedPres) || (step===3 && !selectedVP1) || (step===4 && !selectedVP2) || (step===5 && !selectedVP3)} onClick={handleNextStep} className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-black text-lg shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 text-slate-100 font-sans text-slate-100">다음 단계 <ChevronRight size={18}/></button>
                </div>
              </div>
            )}
            ...
            {step === 6 && (
              <div className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-100 animate-in zoom-in duration-500 text-slate-900">
                ...
                <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100 mb-8 flex items-start gap-3 text-slate-900">
                  ...
                  <p className="text-sm text-blue-900 font-bold leading-relaxed font-sans text-slate-900 text-slate-900">본 투표는 <span className="underline decoration-2 underline-offset-2">비밀 투표</span>로 진행됩니다. 제출 후에는 그 누구도 투표 내용을 확인할 수 없으니 안심하고 제출해 주세요.</p>
                </div>
                <div className="space-y-4 mb-10 text-slate-900">
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 font-black text-center text-lg text-slate-900 text-slate-900">{userData.grade}학년 {userData.class}반 {userData.number}번 {userData.name}</div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900 text-slate-900">
                    <div className="p-5 bg-amber-50 border border-amber-100 rounded-2xl font-black text-slate-900 text-slate-900"><p className="text-[10px] text-amber-600 mb-1 uppercase tracking-widest font-black text-amber-600">전교회장</p>{selectedPres?.name}</div>
                    <div className="p-5 bg-blue-50 border border-blue-100 rounded-2xl font-black text-slate-900 text-slate-900"><p className="text-[10px] text-blue-600 mb-1 uppercase tracking-widest font-black text-blue-600">1학년 부회장</p>{selectedVP1?.name}</div>
                    <div className="p-5 bg-indigo-50 border border-indigo-100 rounded-2xl font-black text-slate-900 text-slate-900"><p className="text-[10px] text-indigo-600 mb-1 uppercase tracking-widest font-black text-indigo-600">2학년 부회장</p>{selectedVP2?.name}</div>
                    <div className="p-5 bg-purple-50 border border-purple-100 rounded-2xl font-black text-slate-900 text-slate-900"><p className="text-[10px] text-purple-600 mb-1 uppercase tracking-widest font-black text-purple-600">3학년 부회장</p>{selectedVP3?.name}</div>
                  </div>
                </div>
                <div className="flex gap-4 text-slate-900">
                  <button onClick={() => setStep(5)} className="flex-1 py-5 bg-slate-100 rounded-2xl font-black transition-all active:scale-95 text-slate-900 font-sans text-slate-900">수정</button>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden relative animate-in slide-in-from-bottom-4 text-left text-slate-900">
              {!!(resetConfirm || showResetAllModal) && (
                <div className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 text-center text-white text-slate-900">
                  <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in text-slate-900 text-slate-900">
                    ...
                    <h4 className="text-xl font-black mb-2 text-slate-900 text-slate-900">{resetConfirm ? '개별 리셋' : '전체 초기화'}</h4>
            ...
            {!isAdminAuthenticated ? (
              <form onSubmit={handleAdminLogin} className="p-10 space-y-4 text-slate-900">
                <input type="text" placeholder="Admin ID" value={adminAuthForm.id} onChange={(e)=>setAdminAuthForm({...adminAuthForm, id: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none text-slate-900 font-sans text-slate-900" />
                <input type="password" placeholder="Password" value={adminAuthForm.pw} onChange={(e)=>setAdminAuthForm({...adminAuthForm, pw: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none text-slate-900 font-sans text-slate-900" />
              ...
              <div className="p-8 space-y-8 animate-in fade-in duration-500 font-sans text-slate-900">
                ...
                {adminTab === 'stats' && (
                  <div className="space-y-10 text-slate-900 text-slate-900">
                    <div className="flex justify-between items-center gap-4 text-slate-900 font-sans text-slate-900">
                      <p className="font-black text-sm text-slate-900 text-slate-900">전체 투표율: <span className="text-blue-600 text-xl font-sans">{((stats.total / Object.keys(FINAL_AUTH_CODES_MAP).length) * 100).toFixed(1)}%</span> ({stats.total}/{Object.keys(FINAL_AUTH_CODES_MAP).length}명)</p>
                      <button onClick={handleExportResults} className="px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl text-xs font-black flex items-center gap-2 shadow-sm text-emerald-600 font-sans text-emerald-600"><Download size={14}/> 엑셀</button>
                    </div>
                      ...
                      {['pres', 'vp1', 'vp2', 'vp3'].map((key) => {
                        ...
                        return (
                          <div key={key} className="space-y-4 text-slate-900 text-slate-900">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b pb-2 text-slate-400">{key==='pres' ? '전교회장' : key==='vp1' ? '1학년 부회장' : key==='vp2' ? '2학년 부회장' : '3학년 부회장'}</p>
                ...
                {adminTab === 'list' && (
                  <div className="space-y-6 text-slate-900 text-slate-900">
                    <div className="flex gap-4 text-slate-900 font-sans text-slate-900">
                      <div className="relative flex-1 text-slate-900 font-sans text-slate-900"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="학번 또는 이름 검색..." className="w-full p-3 pl-9 bg-slate-50 border rounded-xl text-xs font-bold outline-none font-sans text-slate-900" onChange={e=>setSearchQuery(e.target.value)} /></div>
                      <button onClick={handleExportVoters} className="p-3 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl text-xs font-black flex items-center gap-2 shadow-sm text-emerald-600 font-sans text-emerald-600"><Download size={14}/> 명단 엑셀 다운로드</button>
                    </div>
                    <div className="bg-white border rounded-2xl overflow-hidden max-h-[400px] overflow-y-auto shadow-inner text-slate-900 font-sans text-slate-900 text-slate-900">
                      <table className="w-full text-left text-[11px] font-sans text-slate-900 text-slate-900">
                        ...
                        <tbody className="divide-y font-sans text-slate-900 text-slate-900">
                          {filteredParticipationList.map(([id, data]) => {
                            ...
                            return (
                              <tr key={id} className="hover:bg-slate-50 text-slate-900 text-slate-900">
                                ...
                                <td className="p-4 text-center font-sans text-blue-600 font-bold text-blue-600">{data.code}</td>
                                <td className="p-4 text-center whitespace-nowrap text-slate-900 text-slate-900">
                                ...
                                <td className="p-4 text-center font-sans text-[10px] text-slate-500 whitespace-nowrap text-slate-500">
                                  {hasVoted && voterData.votedAt ? (
                                    <span className="flex items-center justify-center gap-1 font-sans text-slate-900 text-slate-900">
                                ...
                                <td className="p-4 text-center flex items-center justify-center gap-3">
                                  {hasVoted && <button onClick={()=>setResetConfirm(voterKey)} className="text-amber-500 hover:scale-110 font-sans text-amber-500"><RotateCcw size={14}/></button>}
                                  <button onClick={async ()=>{if(confirm("삭제하시겠습니까?")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'authVault', id + data.name));}} className="text-rose-500 hover:scale-110 font-sans text-rose-500 text-rose-500"><Trash2 size={14}/></button>
                                </td>
                              </tr>
                            );
                          })}
                    ...
                    {['president', 'vp1', 'vp2', 'vp3'].map((cat) => (
                      <div key={cat} className="bg-slate-50 p-6 rounded-3xl border border-slate-200 text-slate-900 text-slate-900">
                        <div className="flex justify-between items-center mb-4 text-slate-900 text-slate-900">
                          <h4 className="font-black text-sm flex items-center gap-2 text-slate-600 text-slate-600 text-slate-600">
                            <Award size={16} /> 
                            {cat === 'president' ? '전교회장' : cat === 'vp1' ? '1학년 부회장' : cat === 'vp2' ? '2학년 부회장' : '3학년 부회장'}
                          </h4>
                          ...
                          }} className="p-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-white text-white"><Plus size={14} /></button>
                        </div>
                          ...
                          {dbCandidates[cat].map((c, i) => (
                            <div key={c.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 relative group text-slate-900 text-slate-900">
                              ...
                              <div className="space-y-3 text-slate-900 text-slate-900">
                                <div className="flex items-center gap-2 text-slate-900 text-slate-900">
                                  <span className="text-[10px] font-black text-blue-500 w-8 text-blue-500">이름</span>
                                  <input className="flex-1 font-black text-sm border-b border-transparent focus:border-blue-500 outline-none px-1 text-slate-900 font-sans text-slate-900" 
                                    value={c.name} 
                                    onChange={async (e) => {
                                      const updated = { ...dbCandidates };
                                      updated[cat][i].name = e.target.value;
                                      await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated);
                                    }} 
                                  />
                                </div>
                                <div className="flex items-center gap-2 text-slate-900 text-slate-900">
                                  <span className="text-[10px] font-black text-slate-400 w-8 text-slate-400">공약</span>
                                  <input className="flex-1 text-xs font-bold border-b border-transparent focus:border-blue-500 outline-none px-1 text-slate-500 font-sans text-slate-500" 
                                    value={c.slogan} 
                                    onChange={async (e) => {
                                      const updated = { ...dbCandidates };
                                      updated[cat][i].slogan = e.target.value;
                                      await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated);
                                    }} 
                                  />
                                </div>
                ...
                {adminTab === 'system' && (
                  <div className="space-y-6 text-slate-900 text-slate-900">
                    <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white font-sans text-slate-900 text-slate-900">
                      ...
                      <div className="flex gap-4 text-slate-900 text-slate-900">
                    ...
                    <div className="bg-blue-50 p-8 rounded-[2.5rem] border border-blue-100 text-slate-900 text-slate-900 text-slate-900">
                      <div className="flex justify-between items-start mb-6 text-slate-900 text-slate-900 text-slate-900">
                        <div className="text-slate-900 text-slate-900">
                          <h4 className="font-black text-blue-800 text-lg mb-1 flex items-center gap-2 font-sans text-slate-900 text-slate-900"><ClipboardCheck size={20}/> 명단 데이터 동기화</h4>
                          <p className="text-xs text-blue-600 font-bold leading-tight font-sans text-slate-600 text-slate-600 text-slate-600">코드 속 850명 명단을 서버로 강제 전송합니다.</p>
                        </div>
                        <div className="bg-blue-600 text-white px-4 py-2 rounded-2xl font-black text-xs font-sans text-slate-100 text-slate-100">
                    ...
                    <button onClick={() => setShowResetAllModal(true)} className="w-full py-4 bg-rose-50 text-rose-600 border border-rose-200 rounded-3xl font-black text-sm flex items-center justify-center gap-2 hover:bg-rose-100 font-sans text-rose-600 text-rose-600"><Trash2 size={16}/> 서버 집계 초기화</button>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="mt-20 pt-10 border-t border-slate-200 text-center text-slate-900 font-sans text-slate-900">
          {!showAdminPanel && (
            <button onClick={() => setShowAdminPanel(true)} className="flex items-center gap-2 mx-auto text-slate-400 hover:text-blue-600 font-bold text-sm transition-colors uppercase tracking-[0.2em] font-sans text-slate-400 text-slate-400"><BarChart3 size={16} /> Admin Mode</button>
          )}
        </div>
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900 text-slate-900">EMS Terminal V8.5 Final Build</footer>
      </div>
    </div>
  );
}
