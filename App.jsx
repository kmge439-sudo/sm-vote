  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4 font-sans text-slate-900 selection:bg-blue-100 select-none">
      <div className="max-w-3xl w-full">
                ...
                {adminTab === 'stats' && (
                  <div className="space-y-10">
                    <div className="flex justify-between items-center gap-4 text-slate-900 font-sans">
                      <p className="font-black text-sm">전체 투표율: <span className="text-blue-600 text-xl font-sans">{((stats.total / Object.keys(FINAL_AUTH_CODES_MAP).length) * 100).toFixed(1)}%</span> ({stats.total}/{Object.keys(FINAL_AUTH_CODES_MAP).length}명)</p>
                      <button onClick={handleExportResults} className="px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl text-xs font-black flex items-center gap-2 shadow-sm text-emerald-600"><Download size={14}/> 엑셀</button>
                    </div>
        ...
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900 text-slate-900">EMS Terminal V6.8 Final Build</footer>
      </div>
    </div>
  );
}
