...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubVotes = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'votes'), (snap) => {
      setDbVotes(snap.docs.map(doc => doc.data()));
    });
    ...
    const unsubVault = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'authVault'), (snap) => {
      setDbStudents(snap.docs.map(doc => ({ key: doc.id, ...doc.data() })));
    });
  ...
  const handleResetAllVotes = async () => {
    if (!user || !isAdminAuthenticated) return;
    if (!confirm("서버의 모든 데이터(명단 포함)를 완전히 삭제하시겠습니까?")) return;
  ...
  const handleExportResults = () => {
    ...
    ['president', 'vp1', 'vp2', 'vp3'].forEach(cat => {
      dbCandidates[cat]?.forEach(p => rows.push([cat, p.name, stats[cat === 'president' ? 'pres' : cat][p.id] || 0]));
    });
      ...
      <div className="max-w-4xl w-full">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-widest shadow-lg">
            <Lock size={12} /> SECURED SYSTEM V6.7
          </div>
            ...
            {step === 6 && (
              <div className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-100 animate-in zoom-in duration-500 text-slate-900">
                ...
                <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100 mb-8 flex items-start gap-3">
                  ...
                  <p className="text-sm text-blue-900 font-bold leading-relaxed font-sans">본 투표는 <span className="underline decoration-2">비밀 투표</span>로 진행됩니다. 제출 후에는 누구도 투표 내용을 확인할 수 없습니다.</p>
                </div>
            ...
            {!!(resetConfirm || showResetAllModal) && (
              <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 text-center">
                <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in text-slate-900">
                  ...
                  <h4 className="text-xl font-black mb-2 text-slate-900">{resetConfirm ? '개별 리셋' : '서버 전체 초기화'}</h4>
                  <p className="text-sm text-slate-500 font-bold mb-6 text-slate-900">{resetConfirm ? '이 학생의 투표를 삭제할까요?' : '주의! 명단과 투표 결과가 모두 삭제됩니다.'}</p>
              ...
              <div className="p-8 space-y-8 animate-in fade-in duration-500 font-sans">
                ...
                {adminTab === 'stats' && (
                  <div className="space-y-10">
                    <div className="flex justify-between items-center gap-4 text-slate-900">
                      <p className="font-black text-sm">전체 투표율: <span className="text-blue-600 text-xl font-sans">{dbStudents.length > 0 ? ((stats.total / dbStudents.length) * 100).toFixed(1) : 0}%</span> ({stats.total}/{dbStudents.length}명)</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 text-slate-900">
                      {['pres', 'vp1', 'vp2', 'vp3'].map((key) => {
                        const candidatesList = dbCandidates ? dbCandidates[key === 'pres' ? 'president' : key] : [];
                ...
                {adminTab === 'students' && (
                  <div className="space-y-6">
                    <div className="flex gap-4">
                      <div className="relative flex-1 text-slate-900 font-sans"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input placeholder="검색..." className="w-full p-3 pl-9 bg-slate-50 border rounded-xl text-xs font-bold outline-none" onChange={e=>setSearchQuery(e.target.value)} /></div>
                      <button onClick={handleExportVoters} className="p-3 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl text-xs font-black flex items-center gap-2 shadow-sm text-emerald-600"><Download size={14}/> 명단 엑셀</button>
                    </div>
                    <div className="bg-white border rounded-2xl overflow-hidden max-h-[400px] overflow-y-auto shadow-inner text-slate-900 font-sans">
                      {dbStudents.length === 0 ? (
                        <div className="p-10 text-center text-slate-400 font-bold">서버에 등록된 명단이 없습니다. [시스템 설정]에서 동기화하세요.</div>
                      ) : (
                        <table className="w-full text-left text-[11px] font-sans">
                          <thead className="bg-slate-50 font-black sticky top-0 text-slate-900">
                            <tr><th className="p-4">학번/이름</th><th className="p-4 text-center">상태</th><th className="p-4 text-center whitespace-nowrap">투표 시간</th><th className="p-4 text-center">관리</th></tr>
                          </thead>
                          <tbody className="divide-y font-sans">
                            {filteredParticipationList.map(s => (
                              <tr key={s.key} className="hover:bg-slate-50 text-slate-900">
                                <td className="p-4 font-black text-slate-900">{s.key}</td>
                                <td className="p-4 text-center whitespace-nowrap">
                                  <span className={`px-2 py-1 rounded font-black text-[9px] ${dbVoters[s.key] ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>{dbVoters[s.key] ? '완료' : '미참여'}</span>
                                </td>
                                <td className="p-4 text-center font-sans text-[10px] text-slate-500 whitespace-nowrap">
                                  {dbVoters[s.key]?.votedAt ? (
                                    <span className="flex items-center justify-center gap-1">
                                      <Clock size={10} className="text-blue-400" />
                                      {new Date(dbVoters[s.key].votedAt).toLocaleString('ko-KR', { hour12: false, month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                                    </span>
                                  ) : '-'}
                                </td>
                                <td className="p-4 text-center flex items-center justify-center gap-3">
                                  {dbVoters[s.key] && <button onClick={()=>setResetConfirm(s.key)} className="text-amber-500 hover:scale-110"><RotateCcw size={14}/></button>}
                                  <button onClick={async ()=>{if(confirm("삭제?")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'authVault', s.key));}} className="text-rose-500 hover:scale-110"><Trash2 size={14}/></button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                ...
                {adminTab === 'system' && (
                  <div className="space-y-6">
                    ...
                    <div className="bg-blue-50 p-8 rounded-[2.5rem] border border-blue-100">
                      <div className="flex justify-between items-start mb-6 text-slate-900">
                        <div>
                          ...
                          <p className="text-xs text-blue-600 font-bold font-sans">코드 내 감지 학생: {Object.keys(FINAL_AUTH_CODES_MAP).length}명</p>
                        </div>
                      </div>
                    ...
                    <button onClick={() => setShowResetAllModal(true)} className="w-full py-4 bg-rose-50 text-rose-600 border border-rose-200 rounded-3xl font-black text-sm flex items-center justify-center gap-2 hover:bg-rose-100 font-sans text-rose-600"><Trash2 size={16}/> 서버 전체 초기화 (신중히 사용)</button>
                  </div>
                )}
              </div>
            )}
        ...
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900 text-slate-900">EMS Terminal V6.7 Master Final</footer>
      </div>
    </div>
  );
}
