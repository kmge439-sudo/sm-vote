...
import { 
  User, CheckCircle, Award, Users, AlertCircle, Lock, BarChart3, RefreshCw, KeyRound, ShieldCheck, X, Search, RotateCcw, Trash2, AlertTriangle, Fingerprint, ChevronRight, ChevronLeft, ShieldAlert, PlayCircle, StopCircle, Timer, Clock, Trophy, Download, UserPlus, Save, ClipboardCheck, Plus, UserMinus, Edit3, Play, FileUp
} from 'lucide-react';
...
// ==========================================
// [보안 데이터] 850명 명단 완벽 고정 (V10.9 - 김하늘, 박나연, 김지원 및 오타 수정)
// ==========================================
...
const D4 = "1701:고소현:3209,1702:곽나경:5696,1703:구은재:1227,1704:권나현:9955,1705:김다윤:4443,1706:김서연:6733,1707:김시은:2405,1708:김연아:7732,1709:김예은:1563,1710:김윤하:3943,1711:김은서:8862,1712:김주하:6641,1713:류이안:2125,1714:박수하:5559,1715:스미스클로이:9033,1716:신재은:4348,1717:육예지:7632,1718:이서진:1319,1719:이솔민:3430,1720:이연서:5131,1721:이지아:6803,1722:이채연:2266,1723:이하연:9022,1724:장서이:1144,1725:정소윤:4471,1726:조서현:8783,1727:조서현:3362,1728:지서연:6113,1729:차수진:2008,1730:최가온:7742,1731:최예진:4582,1801:강나겸:8941,1802:김가령:1125,1803:김나연:3473,1804:김민채:7802,1805:김서연:2255,1806:김서현:9040,1807:김시현:6675,1808:김예나:4342,1809:김정민:8833,1810:김지윤:5564,1811:김채윤:1219,1812:김효담:7630,1813:박다은:3442,1815:손연주:9922,1816:원다경:2153,1817:윤나경:6805,1818:이다은:1222,1819:이도희:8255,1821:이채윤:4477,1822:장효서:3370,1823:정수아:9029,1824:정유진:1148,1825:채시안:5692,1826:최가은:8786,1827:최라임:2402,1828:최수현:4433,1829:최윤채:6640,1830:팔리로니스해나루이즈:8853,1831:황서율:1275,1832:김민서:7762";
...
export default function App() {
  ...
  const syncAuthVault = async () => {
    if (!confirm(`서버 기본 명단을 갱신하시겠습니까?`)) return;
    setIsSyncing(true);
    try {
      ...
      for (let i = 0; i < entries.length; i += 300) {
        const batch = writeBatch(db);
        entries.slice(i, i + 300).forEach(([id, data]) => {
  ...
  const handleExportResults = () => {
    ...
    ['president', 'vp1', 'vp2', 'vp3'].forEach(cat => {
      ...
      candidatesList?.forEach(p => rows.push([cat, p.name, stats[key][p.id] || 0]));
    });
    const csvContent = "\uFEFF" + rows.map(row => row.join(',')).join('\n');
    const link = document.createElement('a');
    link.href = URL.createObjectURL(new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }));
      ...
      <div className="max-w-3xl w-full text-slate-900">
        <div className="text-center mb-12 text-slate-900">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-[0.2em] shadow-lg text-slate-100">
            <Lock size={12} /> SECURED SYSTEM V10.9
          </div>
        ...
        {!showAdminPanel ? (
          <div className="max-w-2xl mx-auto">
            {step === 1 && (
              <div className="bg-white rounded-[2.5rem] shadow-2xl p-10 border animate-in slide-in-from-bottom-4 duration-500">
                {votingState === 'active' ? (
                  <>
                    <h2 className="text-2xl font-black mb-10 flex items-center gap-4"><Fingerprint size={28} className="text-blue-600" /> 본인 확인 및 인증</h2>
                    ...
                    <input type="text" placeholder="이름" value={userData.name} onChange={(e) => setUserData({...userData, name: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-xl mb-4 outline-none focus:border-blue-500" />
                    ...
                    <div className="mt-10 border-t pt-6 text-center">
                       <button onClick={() => setShowAdminPanel(true)} className="text-slate-300 font-bold text-xs uppercase hover:text-blue-600 transition-colors flex items-center gap-2 mx-auto justify-center"><BarChart3 size={14} /> Admin Mode</button>
                    </div>
                  </>
                ) : (
                  <div className="text-center py-10">
                    <Timer size={64} className="mx-auto text-amber-500 mb-6" /><h2 className="text-2xl font-black mb-4">투표 대기/마감</h2><p className="text-slate-500 font-bold font-sans">지금은 투표 가능한 시간이 아닙니다.</p>
                    <div className="mt-10 border-t pt-6 text-center">
                       <button onClick={() => setShowAdminPanel(true)} className="text-slate-300 font-bold text-xs uppercase hover:text-blue-600 transition-colors flex items-center gap-2 mx-auto justify-center"><BarChart3 size={14} /> Admin Mode</button>
                    </div>
                  </div>
            ...
            {[2, 3, 4, 5].includes(step) && dbCandidates && (
              <div className="animate-in slide-in-from-right-8 duration-500">
                  ...
                  {(dbCandidates[step === 2 ? 'president' : `vp${step - 2}`] || []).map(c => {
                    ...
                    return (
                      <div key={c.id} onClick={() => {
                        ...
                        <div><span className="text-[10px] font-black bg-slate-100 px-3 py-1 rounded-lg uppercase tracking-wider">기호 {c.id % 100}번</span><h3 className="text-xl font-black mt-2">{c.name}</h3><p className="text-slate-500 font-bold text-sm italic">"{c.slogan}"</p></div>
                ...
                <div className="flex gap-4 mt-10">
                  <button onClick={() => setStep(step - 1)} className="flex-1 py-4 bg-white border border-slate-200 rounded-2xl font-black transition-all active:scale-95 text-slate-900 font-sans"><ChevronLeft size={18}/>이전</button>
                  <button disabled={!((step===2 && selectedPres) || (step===3 && selectedVP1) || (step===4 && selectedVP2) || (step===5 && (dbCandidates.vp3.length === 0 || selectedVP3)))} onClick={() => setStep(step + 1)} className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-black text-lg shadow-lg active:scale-95 transition-all text-slate-100">다음 단계 <ChevronRight size={18}/></button>
                </div>
              </div>
            )}

            {step === 6 && (
              <div className="bg-white rounded-[3rem] shadow-2xl p-10 border animate-in zoom-in duration-500">
                ...
                <div className="bg-blue-50 p-5 rounded-2xl border mb-8 flex items-start gap-3">
                  ...
                  <p className="text-sm text-blue-900 font-bold leading-relaxed font-sans">비밀 투표가 보장됩니다. 내용을 확인하고 제출해 주세요.</p>
                </div>
                <div className="space-y-4 mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900">
                    <div className="p-5 bg-amber-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-amber-600 mb-1 uppercase tracking-widest font-black">전교회장</p>{selectedPres?.name}</div>
                    <div className="p-5 bg-blue-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-blue-600 mb-1 uppercase tracking-widest font-black">1학년 부회장</p>{selectedVP1?.name}</div>
                    <div className="p-5 bg-indigo-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-indigo-600 mb-1 uppercase tracking-widest font-black">2학년 부회장</p>{selectedVP2?.name}</div>
                    <div className="p-5 bg-purple-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-purple-600 mb-1 uppercase tracking-widest font-black">3학년 부회장</p>{selectedVP3?.name}</div>
                  </div>
                </div>
                <div className="flex gap-4">
                  <button onClick={() => setStep(5)} className="flex-1 py-5 bg-slate-100 rounded-2xl font-black transition-all">수정</button>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden relative animate-in slide-in-from-bottom-4 text-left">
                ...
                <div className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 text-center text-white text-slate-900">
                  <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in text-slate-900 text-slate-900">
                    ...
                    <div className="flex gap-3 text-slate-900">
                      ...
                      <button onClick={resetConfirm ? () => handleResetVoter(resetConfirm) : handleResetAllVotes} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-bold shadow-lg text-slate-100 font-sans text-slate-100">진행</button>
                    </div>
                  </div>
                </div>
              )}
            ...
            {!isAdminAuthenticated ? (
              <form onSubmit={handleAdminLogin} className="p-10 space-y-4">
                <input type="text" placeholder="Admin ID" className="w-full p-4 bg-slate-50 border rounded-2xl font-bold outline-none font-sans" value={adminAuthForm.id} onChange={(e)=>setAdminAuthForm({...adminAuthForm, id: e.target.value})}/>
                <input type="password" placeholder="Password" className="w-full p-4 bg-slate-50 border rounded-2xl font-bold outline-none font-sans" value={adminAuthForm.pw} onChange={(e)=>setAdminAuthForm({...adminAuthForm, pw: e.target.value})}/>
              ...
              <div className="p-8 space-y-8 animate-in fade-in duration-500 font-sans text-slate-900">
                ...
                {adminTab === 'stats' && (
                  <div className="space-y-10">
                    <div className="flex justify-between items-center gap-4">
                      <p className="font-black text-sm text-slate-700">전체 투표율: <span className="text-blue-600 text-xl font-sans">{(((stats?.total || 0) / (Object.keys(currentAuthCodesMap).length || 1)) * 100).toFixed(1)}%</span> ({(stats?.total || 0)}/{Object.keys(currentAuthCodesMap).length}명)</p>
                ...
                {adminTab === 'list' && (
                  <div className="space-y-6">
                    <div className="flex flex-col md:flex-row gap-4 justify-between items-center font-sans text-slate-900">
                      <div className="relative flex-1 w-full"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="이름/학번 검색..." className="w-full p-3 pl-9 bg-slate-50 border rounded-xl text-xs font-bold outline-none font-sans" onChange={e=>setSearchQuery(e.target.value)} /></div>
                      <div className="flex gap-2 w-full md:w-auto">
                        <label className="flex-1 md:flex-none cursor-pointer p-3 bg-blue-50 text-blue-600 border border-blue-100 rounded-xl text-xs font-black flex items-center justify-center gap-2 shadow-sm text-blue-600">
                          <FileUp size={14}/> CSV 업로드
                          <input type="file" accept=".csv" className="hidden" onChange={handleCSVUpload} />
                        </label>
                        <button onClick={handleExportVoters} className="flex-1 md:flex-none p-3 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl text-xs font-black flex items-center justify-center gap-2 shadow-sm text-emerald-600 font-sans text-emerald-600"><Download size={14}/> 결과 엑셀</button>
                      </div>
                    </div>
                    <div className="bg-white border rounded-2xl overflow-hidden max-h-[400px] overflow-y-auto shadow-inner text-slate-900">
                      <table className="w-full text-left text-[11px] font-sans">
                        <thead className="bg-slate-50 font-black sticky top-0 text-slate-900 font-sans">
                          <tr><th className="p-4">학번/이름</th><th className="p-4 text-center">코드</th><th className="p-4 text-center">상태</th><th className="p-4 text-center">관리</th></tr>
                        </thead>
                          ...
                          {filteredParticipationList.map(([id, data]) => {
                            ...
                            return (
                              <tr key={id} className="hover:bg-slate-50 text-slate-900">
                                <td className="p-4 font-black text-slate-900">{id[0]}-{id[1]}-{id.substring(2)} {data.name}</td>
                                <td className="p-4 text-center font-sans">
                                   <input 
                                     className="w-16 p-1 border rounded text-center text-blue-600 font-bold bg-slate-50 outline-none focus:bg-white focus:border-blue-400 text-blue-600"
                                     value={dbAuthCodes[studentKey] || data.code}
                                     onChange={(e) => updateSingleCode(studentKey, e.target.value)}
                                   />
                                </td>
                                <td className="p-4 text-center whitespace-nowrap text-slate-900">
                                ...
                                <td className="p-4 text-center flex items-center justify-center gap-3">
                                  ...
                                  <button onClick={async ()=>{if(confirm("서버에서 삭제하시겠습니까?")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'authVault', studentKey));}} className="text-rose-500 hover:scale-110 font-sans text-rose-500 text-rose-500"><Trash2 size={14}/></button>
                                </td>
                              </tr>
                            );
                          })}
                ...
                {adminTab === 'candidates' && dbCandidates && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-slate-900">
                    {['president', 'vp1', 'vp2', 'vp3'].map((cat) => (
                      <div key={cat} className="bg-slate-50 p-6 rounded-3xl border border-slate-200">
                        <div className="flex justify-between items-center mb-4 text-slate-900">
                          <h4 className="font-black text-sm flex items-center gap-2 text-slate-600"><Award size={16} /> {cat === 'president' ? '전교회장' : cat === 'vp1' ? '1학년 부회장' : cat === 'vp2' ? '2학년 부회장' : '3학년 부회장'}</h4>
                          ...
                          {dbCandidates[cat].map((c, i) => (
                            <div key={c.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 relative group text-slate-900">
                              ...
                              <div className="space-y-3">
                                <div className="flex items-center gap-2 text-slate-900"><span className="text-[10px] font-black text-blue-500 w-8 text-blue-500">이름</span><input className="flex-1 font-black text-sm border-b border-transparent focus:border-blue-500 outline-none px-1 text-slate-900" value={c.name} onChange={async (e) => { const updated = { ...dbCandidates }; updated[cat][i].name = e.target.value; await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated); }} /></div>
                                <div className="flex items-center gap-2 text-slate-900"><span className="text-[10px] font-black text-slate-400 w-8 text-slate-400">공약</span><input className="flex-1 text-xs font-bold border-b border-transparent focus:border-blue-500 outline-none px-1 text-slate-500" value={c.slogan} onChange={async (e) => { const updated = { ...dbCandidates }; updated[cat][i].slogan = e.target.value; await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated); }} /></div>
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                {adminTab === 'system' && (
                  <div className="space-y-6 text-slate-900">
                    ...
                    <button onClick={syncAuthVault} disabled={isSyncing} className="w-full py-4 bg-blue-600 text-white rounded-3xl font-black shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-transform text-slate-100 font-sans text-slate-100">
                      {isSyncing ? <RefreshCw className="animate-spin" /> : <Save />} 초기 서버 명단 전송
                    </button>
                    ...
                    <button onClick={() => setShowResetAllModal(true)} className="w-full py-4 bg-rose-50 text-rose-600 border border-rose-200 rounded-3xl font-black text-sm flex items-center justify-center gap-2 hover:bg-rose-100 transition-colors text-rose-600"><Trash2 size={16}/> 서버 집계 및 명단 완전 초기화</button>
                  </div>
                )}
              </div>
            )}
        ...
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900">EMS Terminal V10.9 Final Build</footer>
      </div>
    </div>
  );
}
