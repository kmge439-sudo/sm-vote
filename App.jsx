...
export default function App() {
  ...
  const syncAuthVaultAggressive = async () => {
    ...
    if (!confirm("서버의 모든 명단 및 후보 데이터를 지우고 현재 코드의 정확한 명단으로 강제 덮어쓰기 합니다. 진행할까요?")) return;
    setIsSyncing(true);
    try {
      ...
      alert("서버 데이터 완전 초기화 후 강제 갱신 완료! 이제 후보 관리와 명단이 정상적으로 나타납니다.");
  ...
  const handleResetAllVotes = async () => {
    ...
    try {
      ...
      alert("초기화 완료");
    } catch (err) { } finally { setIsResettingAll(false); }
  };
      ...
      <div className="max-w-3xl w-full text-slate-900">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full mb-4 uppercase tracking-widest shadow-lg text-slate-100">
            <Lock size={12} /> SECURED SYSTEM V13.1
          </div>
        ...
        {!showAdminPanel ? (
                ...
                {votingState === 'active' ? (
                  <>
                    ...
                    <div className="mt-10 border-t pt-6 text-center">
                       <button onClick={() => setShowAdminPanel(true)} className="text-slate-300 font-bold text-xs uppercase hover:text-blue-600 transition-colors flex items-center gap-2 mx-auto justify-center text-slate-300 font-sans"><BarChart3 size={14} /> Admin Mode</button>
                    </div>
                  </>
                ) : (
              ...
              <div className="bg-white rounded-[3rem] shadow-2xl p-10 border animate-in zoom-in duration-500 text-slate-900">
                ...
                <div className="space-y-4 mb-10 text-slate-900">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-900">
                    <div className="p-5 bg-amber-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-amber-600 mb-1 uppercase tracking-widest font-black font-sans text-slate-900">전교회장</p>{selectedPres?.name}</div>
                    <div className="p-5 bg-blue-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-blue-600 mb-1 uppercase tracking-widest font-black font-sans text-slate-900">1학년 부회장</p>{selectedVP1?.name}</div>
                    <div className="p-5 bg-indigo-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-indigo-600 mb-1 uppercase tracking-widest font-black font-sans text-slate-900">2학년 부회장</p>{selectedVP2?.name}</div>
                    <div className="p-5 bg-purple-50 border rounded-2xl font-black text-slate-900"><p className="text-[10px] text-purple-600 mb-1 uppercase tracking-widest font-black font-sans text-slate-900">3학년 부회장</p>{selectedVP3?.name}</div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden relative animate-in slide-in-from-bottom-4 text-left text-slate-900">
            ...
            <div className="bg-slate-900 p-8 text-white flex justify-between items-center font-sans text-slate-100">
              <h3 className="text-xl font-black flex items-center gap-3 text-slate-100"><ShieldCheck className="text-emerald-400" /> 관리 대시보드</h3>
              <button onClick={() => {setShowAdminPanel(false); setIsAdminAuthenticated(false);}} className="p-2 bg-white/10 rounded-full hover:bg-white/20 text-slate-100 font-sans"><X size={18}/></button>
            </div>
            ...
            {!isAdminAuthenticated ? (
              <form onSubmit={handleAdminLogin} className="p-10 space-y-4">
                ...
                {adminAuthError && <p className="text-xs text-rose-500 font-black font-sans">{String(adminAuthError)}</p>}
              ...
              <div className="p-8 space-y-8 animate-in fade-in duration-500 font-sans text-slate-900 text-slate-900">
                ...
                {adminTab === 'stats' && (
                  <div className="space-y-10 text-slate-900">
                    <div className="flex justify-between items-center gap-4 text-slate-900">
                      <p className="font-black text-sm text-slate-700">전체 투표율: <span className="text-blue-600 text-xl font-sans text-blue-600">{(((statsCalculated?.total || 0) / (Object.keys(currentAuthCodesMap).length || 1)) * 100).toFixed(1)}%</span> ({(statsCalculated?.total || 0)}/{Object.keys(currentAuthCodesMap).length}명)</p>
                              ...
                              return (
                                <div key={p.id} className="space-y-1.5 text-slate-900">
                                  <div className="flex justify-between text-xs font-black text-slate-700">
                                    <span className="flex items-center gap-1 text-slate-700">{isTop && <Trophy size={12} className="text-amber-500" />}{p.name}</span>
                                    <span className={isTop ? 'text-rose-600 font-sans' : 'font-sans text-slate-900'}>{count}표</span>
                                  </div>
                ...
                {adminTab === 'list' && (
                  <div className="space-y-6 text-slate-900">
                    <div className="flex flex-col md:flex-row gap-4 justify-between items-center font-sans text-slate-900">
                      <div className="relative flex-1 w-full text-slate-900"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-sans" /><input type="text" placeholder="이름/학번 검색..." className="w-full p-3 pl-9 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold outline-none font-sans text-slate-900" onChange={e=>setSearchQuery(e.target.value)} /></div>
                      <div className="flex gap-2 w-full md:w-auto">
                        <label className="flex-1 md:flex-none cursor-pointer p-3 bg-blue-50 text-blue-600 border border-blue-100 rounded-xl text-xs font-black flex items-center justify-center gap-2 shadow-sm text-blue-600 font-sans">
                          <FileUp size={14}/> CSV 일괄 업로드
                          <input type="file" accept=".csv" className="hidden font-sans" onChange={handleCSVUpload} />
                        </label>
                        <button onClick={downloadCSVTemplate} className="flex-1 md:flex-none p-3 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-xl text-xs font-black flex items-center justify-center gap-2 shadow-sm text-indigo-600 font-sans"><FileDown size={14}/> 양식 다운로드</button>
                        <button onClick={handleExportVoters} className="flex-1 md:flex-none p-3 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl text-xs font-black flex items-center justify-center gap-2 shadow-sm text-emerald-600 font-sans"><Download size={14}/> 참여 현황 엑셀</button>
                      </div>
                    </div>
                    <div className="bg-white border rounded-2xl overflow-hidden max-h-[400px] overflow-y-auto shadow-inner text-slate-900">
                      <table className="w-full text-left text-[11px] font-sans text-slate-900">
                        <thead className="bg-slate-50 font-black sticky top-0 text-slate-900">
                          <tr><th className="p-4 text-slate-900 font-sans">학번/이름</th><th className="p-4 text-center text-slate-900 font-sans">코드 직접수정</th><th className="p-4 text-center text-slate-900 font-sans text-slate-900">상태</th><th className="p-4 text-center text-slate-900 font-sans">관리</th></tr>
                        </thead>
                          ...
                          {filteredParticipationList.map(([id, data]) => {
                            ...
                            return (
                              <tr key={id} className="hover:bg-slate-50 text-slate-900">
                                <td className="p-4 font-black text-slate-900 font-sans">{id[0]}-{id[1]}-{id.substring(2)} {data.name}</td>
                                <td className="p-4 text-center font-sans text-blue-600">
                                   <input 
                                     className="w-20 p-1 border rounded text-center text-blue-600 font-bold bg-slate-50 outline-none focus:bg-white focus:border-blue-400 text-blue-600 font-sans"
                                ...
                                <td className="p-4 text-center flex items-center justify-center gap-3">
                                  {hasVoted && <button onClick={()=>setResetConfirm(studentKey)} className="text-amber-500 hover:scale-110 font-sans text-amber-500"><RotateCcw size={14}/></button>}
                                  <button onClick={async ()=>{if(confirm("서버에서 삭제하시겠습니까?")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'authVault', studentKey));}} className="text-rose-500 hover:scale-110 font-sans text-rose-500"><Trash2 size={14}/></button>
                                </td>
                              </tr>
                            );
                          })}
                ...
                {adminTab === 'candidates' && dbCandidates && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-slate-900">
                    {['president', 'vp1', 'vp2', 'vp3'].map((cat) => (
                      <div key={cat} className="bg-slate-50 p-6 rounded-3xl border border-slate-200 text-slate-900">
                        <div className="flex justify-between items-center mb-4">
                          ...
                          }} className="p-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-white"><Plus size={14} /></button>
                        </div>
                        <div className="space-y-4">
                          {(dbCandidates[cat] || []).map((c, i) => (
                            <div key={c.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 relative group text-slate-900">
                              <button onClick={async () => {
                                const updated = { ...dbCandidates };
                                updated[cat] = updated[cat].filter(cand => cand.id !== c.id);
                                await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated);
                              }} className="absolute top-2 right-2 text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity text-slate-300"><UserMinus size={14} /></button>
                              <div className="space-y-3">
                                <div className="flex items-center gap-2 text-slate-900"><span className="text-[10px] font-black text-blue-500 w-8 font-sans">이름</span><input className="flex-1 font-black text-sm border-b border-transparent focus:border-blue-500 outline-none px-1 text-slate-900 font-sans" value={c.name} onChange={async (e) => { const updated = { ...dbCandidates }; updated[cat][i].name = e.target.value; await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated); }} /></div>
                                <div className="flex items-center gap-2 text-slate-900"><span className="text-[10px] font-black text-slate-400 w-8 font-sans">공약</span><input className="flex-1 text-xs font-bold border-b border-transparent focus:border-blue-500 outline-none px-1 text-slate-500 font-sans" value={c.slogan} onChange={async (e) => { const updated = { ...dbCandidates }; updated[cat][i].slogan = e.target.value; await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'candidates'), updated); }} /></div>
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                {adminTab === 'system' && (
                  <div className="space-y-6 text-slate-900 text-slate-900">
                    <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white font-sans text-slate-100">
                      <h4 className="font-black text-lg mb-6 flex items-center gap-2 font-sans text-slate-100 text-slate-100"><StopCircle size={20} className="text-rose-500"/> 투표 원격 제어</h4>
                      <div className="flex gap-4">
                    ...
                    <button onClick={syncAuthVaultAggressive} disabled={isSyncing} className="w-full py-4 bg-rose-600 text-white rounded-3xl font-black shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-transform text-slate-100 font-sans text-slate-100">
                      {isSyncing ? <RefreshCw className="animate-spin" /> : <ShieldAlert />} 서버 데이터 완전 초기화 후 후보/명단 강제 갱신 실행
                    </button>
                    
                    <button onClick={runSimulation} disabled={isSimulating} className="w-full py-4 bg-amber-600 text-white rounded-3xl font-black shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-transform font-sans text-white">
                      {isSimulating ? <RefreshCw className="animate-spin" /> : <PlayCircle />} 849명 랜덤 투표 시뮬레이션 실행
                    </button>
                    
                    <button onClick={() => setShowResetAllModal(true)} className="w-full py-4 bg-rose-50 text-rose-600 border border-rose-200 rounded-3xl font-black text-sm flex items-center justify-center gap-2 hover:bg-rose-100 transition-colors text-rose-600 font-sans"><Trash2 size={16}/> 서버 집계 초기화 (명단은 유지)</button>
                  </div>
                )}
              </div>
            )}
        ...
        <footer className="text-center mt-12 opacity-20 text-[10px] font-black uppercase tracking-[0.4em] font-sans text-slate-900 text-slate-900">EMS Terminal V12.6 Final Build</footer>
      </div>
    </div>
  );
}
